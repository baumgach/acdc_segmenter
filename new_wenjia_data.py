    ...
    def run(self):
        ...
        for folder in os.listdir(self.base_path):
            ...
            if os.path.isdir(folder_path):
                ...
                for file_img in glob.glob(os.path.join(folder_path, 'lvsa_crop??.nii.gz')):

                    print(file_img)

                    file_base = file_img.split('/')[-1].rstrip('.nii.gz')
                    frame_number = int(file_base.lstrip('lvsa_crop'))
                    if frame_number == 0:
                        cardiac_phase = 'ED'
                    else:
                        cardiac_phase = 'ES'

                    file_mask = 'label_crop' + str(frame_number).zfill(2) + '.nii.gz'
                    ...
                    for zz in range(img.shape[2]):
                        ...
                        # mask_cropped = np.where(mask_cropped == 3, [1], mask_cropped_corr_lbls)
...
if __name__ == '__main__':
    ...
    base_path = '/scratch_net/bmicdl03/data/Wenjia_Cardiac/'
    # data_maker = DataMakerACDC('data_288x288.hdf5', base_path, n_x, n_y)
    data_maker = DataMakerACDC('newwenjiadata_288x288.hdf5', base_path, n_x, n_y)
