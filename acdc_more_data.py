    ...
    def run(self):
        ...
        for folder in os.listdir(self.base_path):
            ...
            if os.path.isdir(folder_path):
                ...
                for file in glob.glob(os.path.join(folder_path, 'patient???_frams??.nii.gz')):
                    ...
                    frame_number = int(file_base.split('frams')[-1])

                    real_gt = frame_number in [systole_frame, diastole_frame]

                    if (train_test == 'test') and (not real_gt):
                        print('skipping...')
                        continue
                    ...
                    for zz in range(img.shape[2]):
                        ...
                        if frame_number == systole_frame:
                            cardiac_phase_list[train_test].append(1)  # 1 == systole
                        elif frame_number == diastole_frame:
                        ...
                        real_gt_list[train_test].append(int(real_gt))
        ...
        for tt in ['test', 'train']:
            ...
            hdf5_file.create_dataset('real_gt_%s' % tt, data=np.asarray(real_gt_list[tt], dtype=np.uint8))
...
if __name__ == '__main__':
    ...
    base_path = '/scratch_net/bmicdl03/code/python/ACDC_challenge/newdata_5_5_15'
    data_maker = DataMakerACDC('data_288x288_plusregs.hdf5', base_path, n_x, n_y)
