...
def unet_bn_bottleneck16(images, training):
    ...
    upconv4 = layers.deconv2D_layer_bn(conv5_2, name='upconv4', kernel_size=(4, 4), strides=(2, 2), num_filters=16, training=training)
    concat4 = tf.concat([conv4_2, upconv4], axis=3, name='concat4')

    conv6_1 = layers.conv2D_layer_bn(concat4, 'conv6_1', num_filters=512, training=training)
    conv6_2 = layers.conv2D_layer_bn(conv6_1, 'conv6_2', num_filters=512, training=training)

    upconv3 = layers.deconv2D_layer_bn(conv6_2, name='upconv3', kernel_size=(4, 4), strides=(2, 2), num_filters=16, training=training)
    concat3 = tf.concat([conv3_2, upconv3], axis=3, name='concat3')

    conv7_1 = layers.conv2D_layer_bn(concat3, 'conv7_1', num_filters=256, training=training)
    conv7_2 = layers.conv2D_layer_bn(conv7_1, 'conv7_2', num_filters=256, training=training)

    upconv2 = layers.deconv2D_layer_bn(conv7_2, name='upconv2', kernel_size=(4, 4), strides=(2, 2), num_filters=16, training=training)
    concat2 = tf.concat([conv2_2, upconv2], axis=3, name='concat2')

    conv8_1 = layers.conv2D_layer_bn(concat2, 'conv8_1', num_filters=128, training=training)
    conv8_2 = layers.conv2D_layer_bn(conv8_1, 'conv8_2', num_filters=128, training=training)

    upconv1 = layers.deconv2D_layer_bn(conv8_2, name='upconv1', kernel_size=(4, 4), strides=(2, 2), num_filters=16, training=training)
    concat1 = tf.concat([conv1_2, upconv1], axis=3, name='concat1')
...
def unet_bn_padded(images, training):


    images_padded = tf.pad(images, [[0,0], [92, 92], [92, 92], [0,0]], 'REFLECT')
    ...
    print('conv7_2.shape')
    print(conv7_2.get_shape().as_list())
    ...
    print('conv9_2.shape')
    print(conv9_2.get_shape().as_list())

    pred = layers.conv2D_layer_bn(conv9_2, 'pred', num_filters=NUM_CLASSES, kernel_size=(1,1), activation=layers.no_activation, training=training, padding='VALID')
...
def unet_bn_padded_hack(images, training):
...
def unet_bn_padded_hack_const_padding(images, training):
...
def unet_bn_padded_hack_const_padding_nobnlastlayer(images, training):
    ...
    pred = layers.conv2D_layer(conv9_2, 'pred', num_filters=NUM_CLASSES, kernel_size=(1,1), activation=layers.no_activation, training=training, padding='VALID')

    return pred

def unet_bn_padded_fixed_const_padding(images, training):
